...
{
	scraping_IMDB("http://www.worldofcontrols.com/page/".$cnt."/?s=is2");
}
...
function scraping_IMDB($url) {
	...
	$i=0;
		$query="INSERT INTO `tbl_parts_world` (`parts_number`,`parts_href`, `scrapped_url`) VALUES ";
		foreach($html->find('div[class="post-title"]') as $link_data)
		{	
			foreach($link_data->find('a') as $a_data)
			{
			echo "<br>".$i;
