...
$parts_query="select * from tbl_parts where parts_status=0 order by parts_id asc limit 200";
//echo $parts_query;
...
function scraping_IMDB($url,$pid) {
	...
	$j=0;
	foreach($html->find('div[class="product_left"]') as $content_data)
	{
	...
	$query="update tbl_parts set parts_desc='".$desc."',parts_image='".$img_url."',parts_status='1' 
	where parts_id='".$pid."'";
	//echo $query;
	mysql_query($query);
	$html->clear();
